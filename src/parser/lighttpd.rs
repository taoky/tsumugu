...
impl Parser for LighttpdListingParser {
    fn get_list(&self, client: &Client, url: &Url) -> Result<ListResult> {
        ...
        for element in indexlist.select(&selector) {
            let a = element.select(&Selector::parse("a").unwrap()).next().unwrap();
            let mtime = element.select(&Selector::parse(".m").unwrap()).next().unwrap();
            let size = element.select(&Selector::parse(".s").unwrap()).next().unwrap();
...
mod tests {
    ...
    fn test_buildroot_root() {
        ...
        let items = LighttpdListingParser::default()
            .get_list(&client, &Url::parse("http://localhost:1921/buildroot/").unwrap())
            .unwrap();
        match items {
            ListResult::List(items) => {
                ...
                assert_eq!(last_item.size, Some(FileSize::HumanizedBinary(262.1, SizeUnit::K)));
                assert_eq!(
                    last_item.mtime,
    ...
    fn test_buildroot_subfolder() {
        ...
        match items {
            ListResult::List(items) => {
                ...
                assert_eq!(items[0].size, Some(FileSize::HumanizedBinary(377.5, SizeUnit::K)));
                assert_eq!(
                    items[0].mtime,
                ...
                assert_eq!(items[3].size, Some(FileSize::HumanizedBinary(362.9, SizeUnit::K)));
                assert_eq!(
                    items[3].mtime,
